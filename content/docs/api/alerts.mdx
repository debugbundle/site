...
| `severity_lifecycle_scope` | `string` | No | For `severity_threshold` only: `new_incident`, `incident_regressed`, or `both`. Defaults to `both`; non-threshold rules return `null`. |
...
For example, a severity-threshold rule with `"severity_min": "medium"` and
`"severity_lifecycle_scope": "both"` notifies for medium, high, and critical incidents
when they are first created and when they later regress.
...
| `severity_threshold` | An incident's severity meets or exceeds `severity_min` on the selected lifecycle scope. |
...
Set `severity_lifecycle_scope` to `null` to reset a `severity_threshold` rule to its
default `both` scope; non-threshold rules always return `null` for this field.
