...
| `status` | `string` | — | `active`, `open`, `resolved`, or `regressed`. `active` means open or regressed. |
| `severity` | `string` | — | `low`, `medium`, `high`, or `critical` |
| `first_seen_after` | `string` | — | Include incidents first seen at or after this ISO-8601 timestamp |
| `attention_after` | `string` | — | Include incidents first seen at or after this ISO-8601 timestamp, or regressed at or after it |
...
curl -H "Authorization: Bearer dbundle_mem_xxx" \
  "https://api.debugbundle.com/v1/incidents?project_id=proj_xxx&status=active&severity=high&limit=10"
```
