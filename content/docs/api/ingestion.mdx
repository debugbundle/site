...
Active project capture rules are also evaluated during ingestion. Matching `drop` rules return `"capture_rule_dropped"`, matching sampled-out `sample` rules return `"capture_rule_sampled_out"`, and matching `demote` rules are accepted as context signals so they cannot open or reopen incidents. Exact-fingerprint rules use the canonical server-derived fingerprint shared with worker grouping; client payloads cannot override it.
