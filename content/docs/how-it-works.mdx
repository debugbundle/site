...
| Mobile activity | Android and Swift lifecycle, screens, process foreground/background, crash replay where supported, ANR/process-exit capture on Android, and native HTTP breadcrumbs |
| Probes | Always-on diagnostic ring buffer data (all tiers) |
| Device context | Browser OS/screen/viewport/language preferences plus Android and Swift app/device/runtime facts, with sensitive values redacted before transmission |
...
| **Mobile offline queue** | Android and Swift store bounded app-private event batches and flush when platform lifecycle and connectivity allow | Mobile offline, background, local/development, and retry paths |
...
<WorkflowDiagram>
  <WorkflowStep title="SDKs and Logs" meta="Input">
    Node.js, Browser, Python, PHP, Java, Go, Ruby, Android, Swift, WordPress, or existing log files.
  </WorkflowStep>
