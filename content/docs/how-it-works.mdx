...
| Mobile activity | Android lifecycle, screens, process foreground/background, crash replay, ANR/process-exit capture, native HTTP breadcrumbs |
| Probes | Always-on diagnostic ring buffer data (all tiers) |
| Device context | Browser OS/screen/viewport/language preferences and Android app/device/runtime facts, with sensitive values redacted before transmission |
...
| **Android offline queue** | Android stores bounded app-private event batches and flushes with WorkManager | Mobile offline, background, local/development, and retry paths |
...
<WorkflowDiagram>
  <WorkflowStep title="SDKs and Logs" meta="Input">
    Node.js, Browser, Python, PHP, Java, Go, Ruby, Android, WordPress, or existing log files.
  </WorkflowStep>
  <WorkflowStep title="Local Queues" meta="Local-only">
    Server events land in `.debugbundle/local/events/`; Android uses an app-private offline queue before flush.
  </WorkflowStep>
