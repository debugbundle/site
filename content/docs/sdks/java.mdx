...
Host the browser relay with the matching servlet class:
...
```sh
JAVA_OPTS="$JAVA_OPTS -javaagent:/opt/debugbundle/debugbundle-java-agent-1.1.1.jar=config=/opt/debugbundle/debugbundle.properties,capture-jul=true,capture-uncaught=true"
exec /opt/jboss/wildfly/bin/standalone.sh -b 0.0.0.0
```
