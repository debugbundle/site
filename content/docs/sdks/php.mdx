...
For WordPress, use the [DebugBundle WordPress plugin](/docs/integrations/wordpress) instead of installing Composer packages on the site. The plugin vendors this PHP SDK, bundles the browser SDK, and registers the WordPress REST browser relay for you.
...
The PHP SDK currently includes a browser relay foundation for server-rendered apps that also load `@debugbundle/sdk-browser`.
