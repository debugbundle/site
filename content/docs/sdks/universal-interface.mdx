...
| `file` | Node, Python, PHP, Java, Go, Ruby | Write events to local filesystem for CLI processing. Default for local-only server projects. |
| `offline queue` | Android, Swift | Bounded app-private queue with platform-aware deferred flushing. Used for offline, background, and local/development paths. |
...
| Feature | Node.js | Browser | Python | PHP | Java | Go | Ruby | Android | Swift |
|---------|---------|---------|--------|-----|------|----|------|---------|-------|
| Status | Shipped | Shipped | Shipped | Shipped | Shipped | Shipped | Shipped | Pre-release | Released v0.1.0 |
| Package | `@debugbundle/sdk-node` | `@debugbundle/sdk-browser` | `debugbundle-python` | `debugbundle/sdk-php` | Maven `com.debugbundle` modules | `github.com/debugbundle/debugbundle-go` | `debugbundle` | Maven `com.debugbundle` Android modules | `https://github.com/debugbundle/debugbundle-swift` |
| Framework integrations | Express, Fastify, Next.js | — | Django, Flask, FastAPI | Laravel, Symfony | Spring Boot, servlet, JAX-RS | `net/http`, Gin, Echo | Rails, Rack, Sidekiq | Android runtime, OkHttp, Ktor, Navigation, Compose | iOS runtime, URLSession, Alamofire, UIKit, SwiftUI |
| Logger integrations | pino, winston, bunyan | — | logging, structlog, loguru | Monolog | JUL, Logback | slog, zap, zerolog | Logger, Semantic Logger | Timber | SwiftLog |
| Safe backend/runtime facts | Full process facts | — | Full process facts | Full process facts | Full process facts | Full process facts | Full process facts | Android app/device context | iOS app/device context |
| Breadcrumbs | — | Yes | — | — | — | — | — | Yes | Yes |
| Relay handler | Express, Fastify, Next.js | Relay client path | Django, Flask, FastAPI | Laravel, Symfony | Spring Boot, servlet, JAX-RS | `net/http`, Gin, Echo | Rack middleware + Rails route | N/A | N/A |
| Probes | Yes | Yes | Yes | Yes | Yes | Yes | Yes | Yes | Yes |
| File transport | Yes | — | Yes | Yes | Yes | Yes | Yes | Offline queue | Offline queue |
...
- [Swift SDK](/docs/sdks/swift) — Full Swift setup with SwiftUI, UIKit, URLSession, Alamofire, SwiftLog, offline queueing, crash replay helpers, and probes
