...
Every DebugBundle SDK implements this universal interface contract. Language-specific naming conventions differ (camelCase for JavaScript/PHP/Java/Kotlin, snake_case for Python/Ruby), but the semantics, behavior guarantees, and volume control rules are identical.
...
| `offline queue` | Android | Bounded app-private queue with deferred WorkManager flushing. Used for offline, background, and local/development paths. |
...
- **Local-only** → `file` transport (server SDKs), app-private offline queue (Android), `relay` or `http` (browser)
...
| Feature | Node.js | Browser | Python | PHP | Java | Go | Ruby | Android |
|---------|---------|---------|--------|-----|------|----|------|---------|
| Status | ✅ Shipped | ✅ Shipped | ✅ Shipped | ✅ Shipped | ✅ Shipped | ✅ Shipped | ✅ Shipped | Pre-release |
| Package | `@debugbundle/sdk-node` | `@debugbundle/sdk-browser` | `debugbundle-python` | `debugbundle/sdk-php` | Maven `com.debugbundle` modules | `github.com/debugbundle/debugbundle-go` | `debugbundle` | Maven `com.debugbundle` Android modules |
| Framework integrations | Express, Fastify, Next.js | — | Django, Flask, FastAPI | Laravel, Symfony | Spring Boot, servlet, JAX-RS | `net/http`, Gin, Echo | Rails, Rack, Sidekiq | Android runtime, OkHttp, Ktor, Navigation, Compose |
| Logger integrations | pino, winston, bunyan | — | logging, structlog, loguru | Monolog | JUL, Logback | slog, zap, zerolog | Logger, Semantic Logger | Timber |
| Safe backend/runtime facts | Full process facts | — | Full process facts | Full process facts | Full process facts | Full process facts | Full process facts | Android app/device context |
| Breadcrumbs | — | ✅ | — | — | — | — | — | ✅ |
| Relay handler | Express, Fastify, Next.js | Relay client path | Django, Flask, FastAPI | Laravel, Symfony | Spring Boot, servlet, JAX-RS | `net/http`, Gin, Echo | Rack middleware + Rails route | N/A |
| Probes | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| File transport | ✅ | — | ✅ | ✅ | ✅ | ✅ | ✅ | Offline queue |
