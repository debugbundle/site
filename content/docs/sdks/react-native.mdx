...
The React Native podspec depends on the native `DebugBundle` CocoaPod from the iOS SDK. That pod is published as `DebugBundle` `0.1.1`.
...
| Native dependency floors | Android SDK `com.debugbundle:debugbundle-android-bom:0.1.2`; iOS CocoaPod `DebugBundle ~> 0.1.1` |
...
| iOS | `DebugBundle` iOS SDK CocoaPod for queueing, transport, config/status, crash/error capture handoff, and probes |
...
- [iOS SDK](/docs/sdks/swift) — Native iOS foundation and CocoaPods dependency used by the RN iOS bridge
