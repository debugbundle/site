...
| Capability | Node.js | Browser | Python | PHP | WordPress | Java | Ruby |
|---|---|---|---|---|---|---|---|
| Status | Shipped | Shipped | Shipped | Shipped | Shipped via GitHub Releases | Shipped | Pre-release |
| Package | `@debugbundle/sdk-node` | `@debugbundle/sdk-browser` | `debugbundle-python` | `debugbundle/sdk-php` | `debugbundle-wordpress` | `com.debugbundle:debugbundle-spring-boot-starter` | `debugbundle` |
| Unhandled exceptions | Yes | Yes | Yes | Yes | Yes, through PHP SDK | Yes | Yes |
| Handled exception capture | Yes | Yes | Yes | Yes | Yes, through PHP SDK | Yes | Yes |
| Request 5xx incident capture | Yes | Yes | Yes | Yes | Yes | Yes | Yes |
| Browser first-party 5xx promotion | N/A | Yes | N/A | N/A | Yes, through browser SDK | N/A | N/A |
| Breadcrumbs | No | Yes | No | No | Yes, through browser SDK | No | No |
| Relay support | Full relay handler + adapters | Relay client path | Full relay handler | Full relay handler | WordPress REST relay | Spring Boot relay route | Rack middleware + Rails route |
| Remote config and capture policy | Yes | Yes | Yes | Yes | Yes, backend SDK path | Yes | Yes |
| Probes | Yes | Yes | Yes | Yes | Yes, through bundled SDKs | Yes | Yes |
| Redaction defaults | Yes | Yes | Yes | Yes | Yes | Yes | Yes, with Rails filters |
| Framework integrations | Express, Fastify, Next.js | Browser-native; works with React, Next.js client-side, Vue, Angular, Svelte, and vanilla apps | Django, Flask, FastAPI | Laravel, Symfony | WordPress plugin wrapper | Spring Boot MVC | Rails, Rack, Sidekiq |
| Local file transport | Yes | No | Yes | Yes | Relay spool for browser events | Yes | Yes |
