...
| Capability | Node.js | Browser | Python | PHP | WordPress |
|---|---|---|---|---|---|
| Status | Shipped | Shipped | Shipped | Shipped | Pre-release |
| Package | `@debugbundle/sdk-node` | `@debugbundle/sdk-browser` | `debugbundle-python` | `debugbundle/sdk-php` | `debugbundle-wordpress` |
| Unhandled exceptions | Yes | Yes | Yes | Yes | Yes, through PHP SDK |
| Handled exception capture | Yes | Yes | Yes | Yes | Yes, through PHP SDK |
| Request 5xx incident capture | Yes | Yes | Yes | Yes | Yes |
| Browser first-party 5xx promotion | N/A | Yes | N/A | N/A | Yes, through browser SDK |
| Breadcrumbs | No | Yes | No | No | Yes, through browser SDK |
| Relay support | Full relay handler + adapters | Relay client path | Full relay handler | Full relay handler | WordPress REST relay |
| Remote config and capture policy | Yes | Yes | Yes | Yes | Yes, backend SDK path |
| Probes | Yes | Yes | Yes | Yes | Yes, through bundled SDKs |
| Redaction defaults | Yes | Yes | Yes | Yes | Yes |
| Framework integrations | Express, Fastify, Next.js | Browser-native; works with React, Next.js client-side, Vue, Angular, Svelte, and vanilla apps | Django, Flask, FastAPI | Laravel, Symfony | WordPress plugin wrapper |
| Local file transport | Yes | No | Yes | Yes | Relay spool for browser events |
