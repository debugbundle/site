...
| Value | Behavior |
|-------|----------|
| `off` | Non-5xx request events are dropped; 5xx request failures are still captured as incident signals. |
| `failures_only` | Only requests with 5xx responses. |
| `filtered` | Requests matching configured filters; until custom filters are configured, SDKs treat this as 5xx-only. |
