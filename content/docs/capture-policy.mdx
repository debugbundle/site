...
| Value | Behavior |
|-------|----------|
| `off` | Standalone request events are dropped unless they match the active preset's immediate request-failure statuses; those immediate request failures are still captured as incident signals. |
| `failures_only` | Immediate request failures for the active preset are captured as incident signals: `minimal` = `5xx`; `balanced` = `5xx`, `408`, `423`, `424`, `425`, `429`; `investigative` = balanced plus `409`. Balanced and investigative also capture preset-enabled request-anomaly candidates (`400`, `401`, `403`, `404`, `409`, `410`, `422`) as context signals so repeated failures can open request-anomaly incidents. |
