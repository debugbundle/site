...
If empty, your SDK or log ingestion hasn't written events yet. On a server-local workflow, verify you are checking the same server or mounted volume where the application writes `.debugbundle/local/events/`. See [Local Workflow](/docs/cli/local-workflow) for capture steps.
