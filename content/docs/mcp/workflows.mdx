...
Use this workflow when a user asks how many people use a product, where a journey
breaks down, which routes or devices are affected, or whether an observed incident
...
7. generate_analytics_bundle              → Preserve one bounded analysis question when a durable artifact is useful
```

Start with direct aggregate reads. They answer most usage questions without creating
another retained artifact. Journey samples are structured, redacted event sequences,
not video replay. Generated analytics bundles are for durable, bounded analysis, not one bundle per visit.
...
>
> The agent calls `get_usage_summary` and `get_device_breakdown`, narrows the result with
> `get_funnel_analysis`, and calls `get_incident_impact` for the incident window. It inspects
> a small set of journey samples only if the aggregate drop-off needs supporting evidence.
> It generates an analytics bundle only if the user wants the bounded review retained and shared.
...
```
Setup Verification → Token Rotation → Product Analytics Review → Webhook Setup → Alert Configuration → Availability Check Investigation
```
...
- [AnalyticsBundle](/docs/analytics) — Product analytics model, privacy, retention, and evidence lifecycle
