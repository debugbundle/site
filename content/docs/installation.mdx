...
Browser events are typically shipped through a same-origin relay handler on your backend, which injects the project token server-side. See [Browser Relay Setup](/docs/sdks/browser-relay) for configuration.
...
Across the Node.js, Browser, Python, and PHP SDKs, request failures with 5xx responses are treated as incident signals. On the Browser SDK, first-party 5xx responses also remain visible as breadcrumbs while emitting standalone `request_event` signals for incident creation.
