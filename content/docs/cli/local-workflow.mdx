...
DebugBundle's local workflow gives you the full debugging experience wherever the SDK can write files and the CLI can process them:

```
Capture → Process → Inspect → Resolve
```

No cloud account, no signup, and no network requests to DebugBundle servers. Everything runs on disk.

This can be a development machine or a self-managed server. For server-local use, keep `.debugbundle/` on persistent storage and run `debugbundle process` on that server or on a mounted/synced copy of the same directory.
...
inc_g7h8i9 | high     | regressed | Checkout worker timeout
```
...
debugbundle incidents --status active --severity high
debugbundle incidents --status all
