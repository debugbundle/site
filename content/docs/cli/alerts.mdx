...
| `severity_threshold` | An incident's severity meets or exceeds the configured minimum on selected lifecycle events. |
...
| `--severity-lifecycle-scope` | `string` | `both` for `severity_threshold` | For severity-threshold rules only: `new_incident`, `incident_regressed`, or `both`. |
...
For `severity_threshold`, `--severity-lifecycle-scope both` means the threshold can
notify when a matching incident opens and when it later regresses. Non-threshold
conditions ignore this field because their condition already defines the lifecycle event.
...
debugbundle alert update alt_01HABC... \
  --project-id proj_01HXYZ... \
  --severity-min critical \
  --severity-lifecycle-scope incident_regressed \
  --cooldown 0 \
  --is-enabled true
```
...
| `--severity-lifecycle-scope` | `string` | Update severity-threshold lifecycle scope: `new_incident`, `incident_regressed`, or `both` (use `null` to reset to the default `both`). |
...
`slack connect-url` prints a browser URL for Slack OAuth and channel selection. `slack delete` remains available after downgrade and only succeeds once no alert rule or weekly report still references the destination.
