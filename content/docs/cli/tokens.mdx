...
| **Member token** | `dbundle_mem_...` | CLI, API, MCP operations | Read/manage all project resources |
...
| `--allowed-origin` | `string` | — | Optional repeatable origin allowlist for direct/static browser ingestion. Example: `https://www.example.com`. |
| `--auth-file` | `string` | `~/.debugbundle/auth.json` | Path to auth state file. |
| `--json` | `boolean` | `false` | Output as JSON. |

Use `--allowed-origin` only for project tokens that are intentionally embedded in static browser apps. The same setting is available in the dashboard as **Allowed browser origins** when creating a project token; the dashboard accepts one origin per line or comma-separated. Allowlisted tokens reject requests without a matching `Origin`, so do not reuse them for server SDKs or relay forwarding. The allowlist blocks normal browser requests from other origins, but non-browser clients can spoof `Origin`, so relay transport remains the stronger default when a backend exists.
...
```json
{
  "token": {
    "token_id": "tok_01HMNO...",
    "label": "ci-pipeline",
    "plaintext": "dbundle_mem_a1b2c3d4e5f6...",
    "revoked_at": null
  }
}
```
